...
class MarketWebSocket:
    ...
    async def force_reconnect(self) -> None:
        ...
        Use this when you need fresh book snapshots from the server — for
        example after subscribing to a new set of tokens.  The run() loop
        stays alive, reconnects automatically after reconnect_interval, and
        re-sends the current _subscribed_assets on the new connection, which
        causes the server to push full book snapshots for every token.
        """
        if self._ws is not None:
            try:
                ...
                pass
            # Don't set _ws = None here — _run_loop will detect the closed
            # connection via ConnectionClosed and break naturally.
